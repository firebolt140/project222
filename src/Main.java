    ...
    public static void main(String[] args) throws IOException {
        ...
        //try {
            ...
            while (ifstream.hasNextLine()) { //While there is a next line in the file
                ...
                String holdName = ifstream.next();//Read in until white space is found to a string to hold the name of the player

                stats = ifstream.nextLine(); //Read in the next line to a string to hold the stats
...
//        }
    ...
    public static void leagueLeaders(ArrayList<Player> playerList, int chosenStat){
        ...
        if(firstCount < 3){
            ...
            if(secondBestScore != -100 && secondBestScore != 100000) { //If a second best score was found
                Collections.sort(playerNamesList);  //Sort best player name list
                //If on base percentage or batting average print with 3 decimal places
                //Else print with 0
                if (chosenStat == 6 || chosenStat == 7) {
            ...
            if((firstCount + secondCount) < 3){
                ...
                for(int i = 0; i < playerList.size(); i++){
                    if(chosenStat != 2) {
                        if (thirdBestScore < playerList.get(i).getStats(chosenStat) && playerList.get(i).getStats(chosenStat) != bestScore && playerList.get(i).getStats(chosenStat) != secondBestScore) {
                            thirdBestScore = playerList.get(i).getStats(chosenStat);
                        }
                    }
                    else{
                        if (thirdBestScore > playerList.get(i).getStats(chosenStat) && playerList.get(i).getStats(chosenStat) != bestScore && playerList.get(i).getStats(chosenStat) != secondBestScore) {
                ...
                if(thirdBestScore != -100 && thirdBestScore != 100000) { //If a third best score was found
